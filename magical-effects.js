...
class MagicalEffectsSystem {
    ...
    createClickEffect(x, y) {
        ...
        // Also create a trail burst on click for extra feedback
        // The throttle in createMagicalTrail will handle frequency
        this.createMagicalTrail(x, y);
        ...
        // Create main bubble ring (8 bubbles for more lush effect)
        for (let i = 0; i < 8; i++) {
            const angle = (i / 8) * Math.PI * 2;
            const distance = 40 + Math.random() * 20;
            const bubbleSize = 8 + Math.random() * 8;
            ...
            this.effects.push({
                ...
                opacity: 1,
                life: 1,
                maxLife: 45, // Slightly longer life
                type: 'bubble-click',
                rotation: angle,
                rotationSpeed: (Math.random() - 0.5) * 0.1,
                initialSize: bubbleSize,
                pulsePhase: Math.random() * Math.PI * 2,
                bouncePhase: 0,
                gravity: 0.05 // Add gravity for movement
            });
        }
    ...
    updateEffect(effect) {
        ...
        if (effect.type === 'bubble-click') {
            // Main click bubbles with bouncy movement toward target and then drifting
            const progress = 1 - effect.life;

            // Apply gravity and friction
            effect.vy += effect.gravity || 0.05;
            effect.vx *= 0.96;
            effect.vy *= 0.96;

            // Move toward target position with decreasing force
            // Using a more stable approach for bubble-click positioning
            const lerpSpeed = 0.1;
            effect.x += (effect.targetX - effect.x) * lerpSpeed * effect.life;
            effect.y += (effect.targetY - effect.y) * lerpSpeed * effect.life;

            // Bounciness
            effect.bouncePhase += 0.15;
            const bounce = Math.sin(effect.bouncePhase) * 0.5 * effect.life;
            effect.y += bounce;

            // Pulsing size effect
            effect.pulsePhase += 0.2;
            const pulse = Math.sin(effect.pulsePhase) * 0.3 + 1;
            effect.size = effect.initialSize * pulse * (0.4 + effect.life * 0.6);
